...
public class OTObject : MonoBehaviour
{
    ...
    void CheckInputCollidable()
    {
        ...
        if (collidable)
        {
            if (colliderType == ColliderType.Box)
            {
                ...
                if (sp != null)
                    DestroyImmediate(sp);
				otCollider = GetComponent<Collider>();
            }
            else
            {
                if (sp == null)
    ...
    public virtual void Assign(OTObject protoType)
    {
		...
		{
			(otCollider as BoxCollider).center = (protoType.GetComponent<Collider>() as BoxCollider).center;
			(otCollider as BoxCollider).size = (protoType.GetComponent<Collider>() as BoxCollider).size;
			(otCollider as BoxCollider).enabled = (protoType.GetComponent<Collider>() as BoxCollider).enabled;
		}
		else
		if (protoType.GetComponent<Collider>() != null && protoType.GetComponent<Collider>() is SphereCollider)
		{
			(otCollider as SphereCollider).center = (protoType.GetComponent<Collider>() as SphereCollider).center;
			(otCollider as SphereCollider).radius = (protoType.GetComponent<Collider>() as SphereCollider).radius;
			(otCollider as SphereCollider).enabled = (protoType.GetComponent<Collider>() as SphereCollider).enabled;
		}
			...
			if (sa.Length>0)
			{
				...
				while (idx <sa.Length)
				{
					...
					if (o!=null)
						...
						g = g.transform.Find(sa[idx]).gameObject;
					if (g==null)
						return null;
					idx++;
				}				
